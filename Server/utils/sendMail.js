...
export async function sendMail(order) {
  try {
    ...
    const mailOptions = {
      from: '"GloboGreen" <globogreenmobile@gmail.com>', // Sender
      to: "globogreenmobile@gmail.com", // Your email to receive order details
      ...
      html: `
        ...
        <p><strong>Order Subtotal:</strong> ₹${order.subTotalAmt}</p>
        <p><strong>Order Grand Total:</strong> ₹${order.totalAmt}</p>
        <h3>Order Items:</h3>
        <ul>
          ${
